...
# Description: Script to clone GitHub repositories belonging to the GitHub user 'utono'.
...
# Initialize SSH for cloning via SSH
initialize_ssh() {
    ...
    if [ -f "$HOME/.ssh/id_ed25519" ] || [ -f "$HOME/.ssh/id_rsa" ]; then
        echo "🔑 Adding available SSH keys to the agent..."
        ssh-add ~/.ssh/id_ed25519 2>/dev/null || ssh-add ~/.ssh/id_rsa 2>/dev/null
    else
        echo "⚠️ Warning: No SSH key found in ~/.ssh/id_ed25519 or ~/.ssh/id_rsa."
        echo "You may have to create one using 'ssh-keygen -t ed25519' or 'ssh-keygen -t rsa'."
    fi
}

# Clone repositories belonging to 'utono'
clone_repositories() {
